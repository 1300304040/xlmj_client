cc.Class({
    ...
    onGameOver(data){
        ...
        RsubScore.getComponent(cc.Label).string = parseInt(0);
        var LsubScore = cc.find("Canvas/game/left/seat/subScore");
        LsubScore.getComponent(cc.Label).string = parseInt(0);
        var UsubScore = cc.find("Canvas/game/up/seat/subScore");
        UsubScore.getComponent(cc.Label).string = parseInt(0);
        var MsubScore = cc.find("Canvas/game/myself/seat/subScore");
        MsubScore.getComponent(cc.Label).string = parseInt(0);
        // 牌局结束隐藏箭头
        var arrowTip = cc.find("Canvas/arrowTip");
        for(var ii=0; ii<arrowTip.children.length; ii++){
            arrowTip.children[ii].active = false;
        }
        // 牌局结束隐藏胡牌面板
        var huPanel = cc.find('Canvas/game/myself/chatingsss');
        huPanel.active = false;


        // var chaTingBtn = cc.find("Canvas/chaTingBtn");
        //  chaTingBtn.active = false;
        // 牌局结束分数的动画失效
        var RScore = cc.find("Canvas/game/right/seat/score").children; 
        var LScore = cc.find("Canvas/game/left/seat/score").children;
        var UScore = cc.find("Canvas/game/up/seat/score").children;
        var MScore = cc.find("Canvas/game/myself/seat/score").children;
        for(var n=0;n<MScore.length;n++){
            RScore[n].active = false;
            LScore[n].active = false;
            UScore[n].active = false;
            MScore[n].active = false;
        }
        // 游戏结束花猪提示隐藏
        var huaZhuTip = cc.find("Canvas/huaZhuTip");
        huaZhuTip.active = false;
        if(cc.vv.gameNetMgr.conf.type == "xzdd"){
        ...
        for(var i = 0; i < 4; ++i){
            ...
            for(var j = 0; j < userData.huinfo.length; ++j){
                ...
                if(!info.ishupai){
                    ...
                    else if(info.action == "beichahuazhu"){
                        str="被查花猪";
                    }
                    else{
                        str = "被查大叫";
    ...
    onBtnReadyClicked:function(){
        ...
        if(this._isGameEnd){
            var animCtrl = this._gameresult.getComponent(cc.Animation);
        ...
        this._gameover.active = false;
         ...
         var chaTingBtn = cc.find("Canvas/chaTingBtn");
         chaTingBtn.active = false;
        // var toggleBtnTop = cc.find("Canvas/toggleBtnTop");
        // var slideMenuBg = cc.find("Canvas/slideMenuBg");
        // var btn_sqjsfj = c.find("Canvas/btn_sqjsfj");
